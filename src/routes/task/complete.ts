...
const SCHEMA = Joi.object({
    ...
    return_updated_tasks: Joi.string()
        .allow("ALL", "PINNED", "NONE")
        .required(),
});
...
export default async (req: express.Request, res: express.Response) => {
    ...
    if (isCompleted(task)) {
        // if it is already completed then uncomplete it by deleting the record of it being completed
        await prisma.task_completions
            .deleteMany({
                where: {
                    task_id: data.id,
                    completed_at: {
            ...
            .catch((e) => {
                ...
                error(
                    res,
                    500,
                    "Something unexpected happened when uncompleting your task. Please try again.",
                );
            });
    } else {
        // get a unique id
        ...
        await prisma.task_completions
            ...
            .catch((e) => {
                ...
                error(
                    res,
                    500,
                    "Something unexpected happened when completing your task. Please try again.",
                );
            });
    }
    ...
    if (
        data.return_updated_tasks === "ALL" ||
        data.return_updated_tasks === "PINNED"
    ) {
