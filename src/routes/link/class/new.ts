...
export default async (req: express.Request, res: express.Response) => {
    ...
    while (!unique) {
        id = uuidv4()
        unique = (await prisma.link_classes.findMany({
            where: {
                id
            }
        })).length === 0
    ...
    await prisma.link_classes.create({
        data: {
            id: id,
        ...
        console.log(e)
        error(res, 400, "An error occured: " + e.meta.cause || "Could not create new link class.")
    })
    ...
    success(res, updatedTypes, "Successfully created link class.", 200)
}