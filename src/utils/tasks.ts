...
export const calcThreshold = (task: RawTask) => {
    ...
    if (task.repeat_period === "WEEK") {
        ...
        const creationTimestamp = Math.round(
            Date.parse(task.created_at).valueOf() / 1000,
        );
        const weekOfCreationEpoch =
            creationTimestamp - ((creationTimestamp - EPOCH) % SECONDS_IN_WEEK);
        ...
        const startOfMonthISO =
            currentISO.split("-").slice(0, 2).join("-") + "-01T00:00:00.000Z";

        const startOfMonthTimestamp = Math.round(
            new Date(startOfMonthISO).getTime() / 1000,
        ); // convert to seconds
...
export const calcEndThreshold = (task: RawTask) => {
    ...
    if (task.repeat_period === "WEEK") {
        ...
        const startOfNextMonthISO =
            year.toString() + "-" + month.toString() + "-01T00:00:00.000Z";

        const startOfNextMonthTimestamp = Math.round(
            new Date(startOfNextMonthISO).getTime() / 1000,
        ); // convert to seconds
...
export const addCompletionData = (tasks: RawTask[]) => {
    ...
    tasks.forEach((task, _) => {
        ...
        if (
            ...
            task.task_completions.length > 0
        )
            return false;
        filtered.push({
            ...task,
